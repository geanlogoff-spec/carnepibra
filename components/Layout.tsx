...
interface LayoutProps {
  ...
  activeTab: 'members' | 'dashboard' | 'management' | 'reports' | 'settings' | 'users';
  setActiveTab: (tab: 'members' | 'dashboard' | 'management' | 'reports' | 'settings' | 'users') => void;
  merchantName: string;
  onLogout: () => void;
  userRole?: 'admin' | 'manager' | 'viewer';
}

export const Layout: React.FC<LayoutProps> = ({ children, activeTab, setActiveTab, merchantName, onLogout, userRole = 'manager' }) => {
  const [isSidebarOpen, setIsSidebarOpen] = useState(false);

  // Definição dos itens de menu base
  const allNavItems = [
    {
