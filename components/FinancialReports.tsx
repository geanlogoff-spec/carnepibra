...
export const FinancialReports: React.FC<FinancialReportsProps> = ({ carnes }) => {
  ...
  const pendingAmount = allInstallments.filter(i => i.status === 'pending').reduce((acc, curr) => acc + curr.amount, 0);
  
  // Cálculo de Atrasados (Overdue)
  const overdueInstallments = allInstallments.filter(i => i.status === 'pending' && new Date(i.dueDate) < now);
  ...
  const statusCounts = {
    ...
    pending: allInstallments.filter(i => i.status === 'pending' && new Date(i.dueDate) >= now).length,
  ...
  const monthlyFlow = allInstallments.reduce((acc: any, inst) => {
    ...
    if (!acc[monthYear]) acc[monthYear] = { month: monthYear, total: 0, paid: 0, overdue: 0, sortKey: date.getTime() };
    acc[monthYear].total += inst.amount;
    if (inst.status === 'paid') acc[monthYear].paid += inst.amount;
    else if (new Date(inst.dueDate) < now) acc[monthYear].overdue += inst.amount;
  ...
  const customerStats = carnes.reduce((acc: any, carne) => {
    if (!acc[carne.customer.name]) {
      acc[carne.customer.name] = { 
        name: carne.customer.name, 
        total: 0, 
        paid: 0, 
        installments: 0,
        paidCount: 0 
    ...
    acc[carne.customer.name].total += carne.totalAmount;
    acc[carne.customer.name].installments += carne.installments.length;
    acc[carne.customer.name].paid += carne.installments.filter(i => i.status === 'paid').reduce((sum, inst) => sum + inst.amount, 0);
    acc[carne.customer.name].paidCount += carne.installments.filter(i => i.status === 'paid').length;
    return acc;
  }, {});
  ...
  if (carnes.length === 0) {
    return (
      <div className="bg-white rounded-[32px] p-20 text-center border border-dashed border-slate-200">
        <h3 className="text-xl font-black text-slate-800">Sem dados para análise</h3>
        <p className="text-slate-500 mt-2">Gere carnês para visualizar o desempenho financeiro aqui.</p>
      </div>
    );
  }
    ...
    <div id="financial-report" className="space-y-10 animate-in fade-in duration-500 pb-20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="bg-slate-900 p-8 rounded-[32px] text-white shadow-2xl">
          <p className="text-[10px] font-black text-indigo-400 uppercase tracking-widest mb-1">Faturamento Total</p>
          <p className="text-3xl font-black">R$ {totalAmount.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p>
          <p className="text-[10px] text-slate-400 mt-2 font-bold uppercase">{allInstallments.length} Parcelas Emitidas</p>
        </div>
        <div className="bg-white p-8 rounded-[32px] border border-slate-100 shadow-sm">
          <p className="text-[10px] font-black text-emerald-500 uppercase tracking-widest mb-1">Total Liquidado</p>
        ...
        <div className="bg-white p-8 rounded-[32px] border border-slate-100 shadow-sm">
          <p className="text-[10px] font-black text-rose-500 uppercase tracking-widest mb-1">Inadimplência (Atraso)</p>
          <p className="text-3xl font-black text-slate-900">R$ {overdueAmount.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p>
          <p className="text-[10px] text-rose-400 mt-2 font-bold uppercase">{overdueInstallments.length} Parcelas Vencidas</p>
        </div>
        <div className="bg-white p-8 rounded-[32px] border border-slate-100 shadow-sm">
          <p className="text-[10px] font-black text-indigo-600 uppercase tracking-widest mb-1">Ticket Médio</p>
          <p className="text-3xl font-black text-slate-900">R$ {(totalAmount / allInstallments.length).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        ...
        <div className="bg-white rounded-[40px] p-10 border border-slate-100 shadow-sm lg:col-span-1">
          <h3 className="font-black text-slate-900 text-xl mb-8">Saúde da Carteira</h3>
          <div className="space-y-6">
            <div className="flex items-center gap-4">
              <div className="flex-1">
                <div className="flex justify-between text-[10px] font-black uppercase mb-1">
                  <span className="text-emerald-600">Liquidados ({statusCounts.paid})</span>
                  <span>{((statusCounts.paid / allInstallments.length) * 100).toFixed(0)}%</span>
                </div>
                <div className="h-3 bg-slate-50 rounded-full overflow-hidden">
                  <div className="h-full bg-emerald-500 rounded-full" style={{ width: `${(statusCounts.paid / allInstallments.length) * 100}%` }}></div>
                </div>
              </div>
            </div>
            <div className="flex items-center gap-4">
              <div className="flex-1">
                <div className="flex justify-between text-[10px] font-black uppercase mb-1">
                  <span className="text-amber-500">Pendentes ({statusCounts.pending})</span>
                  <span>{((statusCounts.pending / allInstallments.length) * 100).toFixed(0)}%</span>
                </div>
                <div className="h-3 bg-slate-50 rounded-full overflow-hidden">
                  <div className="h-full bg-amber-400 rounded-full" style={{ width: `${(statusCounts.pending / allInstallments.length) * 100}%` }}></div>
                </div>
              </div>
            </div>
            <div className="flex items-center gap-4">
              <div className="flex-1">
                <div className="flex justify-between text-[10px] font-black uppercase mb-1">
                  ...
                  <span>{((statusCounts.overdue / allInstallments.length) * 100).toFixed(0)}%</span>
                </div>
                <div className="h-3 bg-slate-50 rounded-full overflow-hidden">
                  <div className="h-full bg-rose-500 rounded-full" style={{ width: `${(statusCounts.overdue / allInstallments.length) * 100}%` }}></div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="mt-10 pt-8 border-t border-slate-50">
             <div className="bg-indigo-50 p-6 rounded-[24px]">
                <p className="text-[10px] font-black text-indigo-600 uppercase mb-2">Conclusão</p>
                <p className="text-xs font-bold text-slate-700 leading-relaxed">
                  {overdueRate > 15 ? 'Atenção! Sua taxa de atraso está acima da média (15%). Considere revisar sua política de crédito.' : 'Sua carteira de recebíveis apresenta um comportamento saudável.'}
                </p>
             </div>
          </div>
        </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="text-left text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100">
                  ...
                  <th className="pb-4 text-right">Total Previsto</th>
                  <th className="pb-4 text-right">Realizado (Pago)</th>
                  <th className="pb-4 text-right">Atrasado</th>
                  ...
                  <tr key={idx} className="group hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="py-4 text-right">
                       <div className="w-20 ml-auto h-2 bg-slate-100 rounded-full overflow-hidden">
                          <div className="h-full bg-indigo-500" style={{ width: `${(data.paid / data.total) * 100}%` }}></div>
                       </div>
                    </td>
                  </tr>
                ))}
               ...
               {sortedCustomers.map((c: any, idx) => (
                 <tr key={idx} className="hover:bg-indigo-50/30 transition-colors">
                   ...
                   <td className="p-4 text-center">
                      <span className={`text-[10px] font-black px-2 py-1 rounded-full ${((c.paid / c.total) * 100) >= 100 ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-500'}`}>
                        {((c.paid / c.total) * 100).toFixed(0)}%
                      </span>
                   </td>
                 </tr>
               ))}
      ...
      <div className="hidden print:block border-t border-slate-200 mt-20 pt-10 text-center">
         ...
         <p className="text-[8px] text-slate-300 mt-1 uppercase">CarnêPIB.RA - Sistema de Gestão Financeira Inteligente</p>
      </div>
    </div>
  );
};
